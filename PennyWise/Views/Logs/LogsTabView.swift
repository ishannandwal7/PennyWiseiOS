    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                ...
                Divider()
                LogListView(predicate: PennyWiseLog.predicate(with: Array(selectedCategories), searchText: searchText), sortDescriptor: ExpenseLogSort(sortType: sortType, sortOrder: sortOrder).sortDescriptor)
            }
